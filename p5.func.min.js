(function (root, factory) {
  ...
  p5.Gen.prototype.harmonics = function (_x, _args) {
    ...
    if (
      !Array.isArray(_x) &&
      _x.constructor !== Float32Array &&
      _x.constructor !== Float64Array
    ) {
      _x = [_x];
      u = false;
    }
    ...
    else if (_x.constructor === Float32Array)
      _sum = new Float32Array(_x.length);
    else if (_x.constructor === Float64Array)
      _sum = new Float64Array(_x.length);
    for (var i in _x) {
      var j = _args.length;
      _sum[i] = 0;
      while (j--) {
  ...
  p5.Gen.prototype.triples = function (_x, _args) {
    ...
    for (i in _x) {
      ...
      for (var j = _args.length - 1; j > 0; j -= 3) {
        if (_args[j - 1] != 0) {
          var val;
          if (_args[j - 2] == 0) val = 1;
          else {
            val = sin(TWO_PI * (_x[i] / (1 / _args[j - 2]) + _args[j] / 360));
          }
  ...
  p5.Gen.prototype.random = function (_x, _type) {
    ...
    if (!_x) {
      ...
      if (
        ...
        arguments[0].constructor !== Float32Array &&
        arguments[0].constructor !== Float64Array
      ) {
        _x = [_x];
        u = false;
      }
    ...
    if (_x[0] === -1) randomSeed(millis() * 1e5);
    for (var i in _x) {
      if (_x[i] != -1) randomSeed(_x[i] * 1e5);
      switch (_type) {
        case "linear":
        case "even":
          _v[i] = random();
  ...
  p5.Gen.prototype.window = function (_x, _type, _args) {
    ...
    switch (_type) {
      ...
      case "fejér":
      case "triangle":
        for (i in _x) _y[i] = 1 - abs((_x[i] - 0.5) / 0.5);
        break;
      case "bartlett-hann":
        var a0 = 0.62;
        var a1 = 0.48;
        var a2 = 0.38;
        for (i in _x)
          _y[i] = a0 - a1 * abs(_x[i] - 0.5) - a2 * cos(2 * PI * _x[i]);
        break;
      case 4:
      case "blackman":
        var a0 = 7938 / 18608;
        ...
        var a2 = 1430 / 18608;
        for (i in _x)
          _y[i] = a0 - a1 * cos(2 * PI * _x[i]) + a2 * cos(4 * PI * _x[i]);
        break;
      case "generalizedblackman":
        if (!_args[0]) _args[0] = 0.5;
        var _a = _args[0];
        var a0 = (1 - _a) / 2;
        var a1 = 0.5;
        var a2 = _a / 2;
      ...
      case "kaiser":
        var alpha = 3;
        for (i in _x) {
          var above = PI * alpha * sqrt(1 - (2 * _x[i] - 1) * (2 * _x[i] - 1));
          var below = PI * alpha;
      ...
      case "sync":
      case "lanczos":
        for (i in _x) _y[i] = sinc(2 * _x[i] - 1);
        break;
      case "flattop":
        var a0 = 1;
        var a1 = 1.93;
      ...
      case "tukey":
        ...
        for (i in _x) {
          _y[i] = 1;
          if (_x[i] <= ah) {
            _y[i] = 0.5 * (1 + cos(PI * ((2 * _x[i]) / _a - 1)));
          } else if (_x[i] > omah) {
            _y[i] = 0.5 * (1 + cos(PI * ((2 * _x[i]) / _a - 2 / _a + 1)));
          }
        }
        break;
      case "slidinggaussian":
        if (!_args[0]) _args[0] = 0.5;
        if (!_args[1]) _args[1] = 0.4;
        var dx = 2 * (_args[0] - 0.5);
        var sigma = _args[1] * 2;
        for (var i in _x)
          _y[i] = exp(0 - sq(_x[i] * 2 - 1 - dx) / (2 * sigma * sigma));
        break;
      case "adjustablecosine":
        ...
        for (i in _x) {
          _y[i] = 1;
          if (_x[i] <= _a) {
            _y[i] = 0.5 * (1 + cos(PI * (_x[i] / _a - 1)));
          } else {
            _y[i] = 0.5 * (1 + cos(PI * ((_x[i] - _a) / (1 - _a))));
          }
        }
        break;
      case "elliptic":
        if (!_args[0]) _args[0] = 0.5;
        var _a = _args[0];
        var min_param_a = 0 + Number.EPSILON;
        var max_param_a = 1 - Number.EPSILON;
        _a = constrain(_a, min_param_a, max_param_a);
        for (i in _x) {
          _y[i] = 0;
          if (_x[i] <= _a) {
            _y[i] = (1 / _a) * sqrt(sq(_a) - sq(_x[i] - _a));
          } else {
            _y[i] = (1 / (1 - _a)) * sqrt(sq(1 - _a) - sq(_x[i] - _a));
          }
        }
        break;
      case "hyperelliptic":
        if (!_args[0]) _args[0] = 0.5;
        if (!_args[1]) _args[1] = 3;
        var _a = _args[0];
        var _n = _args[1];
        var min_param_a = 0 + Number.EPSILON;
        var max_param_a = 1 - Number.EPSILON;
        ...
        for (i in _x) {
          ...
          if (_x[i] <= _a) {
            _y[i] =
              (1 / _a) * pow(pow(_a, pwn) - pow(_x[i] - _a, pwn), 1 / pwn);
          } else {
            _y[i] =
              (1 / (1 - _a)) *
              pow(pow(1 - _a, pwn) - pow(_x[i] - _a, pwn), 1 / pwn);
          }
        }
        break;
      case "squircular":
        ...
        for (i in _x) {
          ...
          var pwn = max(2, _n * 2);
          if (_x[i] <= _a) {
            _y[i] = 1 - _a + pow(pow(_a, pwn) - pow(_x[i] - _a, pwn), 1 / pwn);
          } else {
            _y[i] = _a + pow(pow(1 - _a, pwn) - pow(_x[i] - _a, pwn), 1 / pwn);
          }
        }
        break;
      case "poisson":
        ...
        for (var i in _x) _y[i] = exp(0 - abs(_x[i] - 0.5) * (1 / tau));
        break;
      case "hann-poisson":
      case "poisson-hann":
      case "hannpoisson":
      case "poissonhann":
        if (!_args[0]) _args[0] = 0.5;
        var tau =
          25 * max(_args[0] * _args[0] * _args[0] * _args[0], Number.EPSILON);
        for (i in _x) {
          var hy = 0.5 * (1 - cos(TWO_PI * _x[i]));
          var py = exp(0 - abs(_x[i] - 0.5) * (1 / tau));
          _y[i] = hy * py;
        }
        break;
      ...
      case "slidingpoissonhann":
        if (!_args[0]) _args[0] = 0.5;
        if (!_args[1]) _args[1] = 0.5;
        var tau =
          25 * max(_args[1] * _args[1] * _args[1] * _args[1], Number.EPSILON);
        for (i in _x) {
          var newx = constrain(_x[i] + (0.5 - _args[0]), 0, 1);
          var hy = 0.5 * (1 - cos(TWO_PI * newx));
          var py = exp(0 - abs(newx - 0.5) * (1 / tau));
          _y[i] = hy * py;
        }
        break;
  ...
  p5.Gen.prototype.fillArray = function (_algo, _len, _args, _seed) {
    ...
    if (_algo === "random") {
      for (var i = 0; i < _len; i++) {
        if (_seed) _p[i] = (i / (_len - 1)) * 1e4 + _seed;
        else _p[i] = "-1";
      }
    } else {
      for (var i = 0; i < _len; i++) {
  ...
  p5.Ease.prototype.elasticInOut = function (_x) {
    if (_x < 0.5) {
      ...
      return (
        0.5 *
        (sin(-13 * HALF_PI * (2 * _x - 1 + 1)) * pow(2, -10 * (2 * _x - 1)) + 2)
      );
    }
  };
  ...
  p5.Ease.prototype.bounceOut = function (_x) {
    if (_x < 4 / 11) {
      ...
      return (363 / 40) * _x * _x - (99 / 10) * _x + 17 / 5;
    } else if (_x < 9 / 10) {
      return (4356 / 361) * _x * _x - (35442 / 1805) * _x + 16061 / 1805;
    } else {
      return (54 / 5) * _x * _x - (513 / 25) * _x + 268 / 25;
    }
  };
  ...
  p5.Ease.prototype.quadraticBezierStaircase = function (_x, _a, _n) {
    ...
    if (_x <= p0x && _x >= x0) {
      if (floor(_x * _n) <= 0) {
        ...
        var za = (x0 - (p1x - 1 / _n)) / denom;
        var zb = (y0 - (p1y - 1 / _n)) / denom;
        var zx = (_x - (p1x - 1 / _n)) / denom;
        var om2a = 1 - 2 * za;
        var interior = max(0, za * za + om2a * zx);
        var t = (sqrt(interior) - za) / om2a;
        var zy = (1 - 2 * zb) * (t * t) + 2 * zb * t;
        zy *= p1y - p0y;
        zy += p1y;
        if (_x > x0) {
          zy -= 1 / _n;
  ...
  p5.Ease.prototype.doubleExponentialSigmoid = function (_x, _a) {
    ...
    if (_x <= 0.5) {
      ...
      _y = 1 - pow(2 * (1 - _x), 1 / _a) / 2;
    }
    return _y;
  };
  p5.Ease.prototype.adjustableCenterDoubleExponentialSigmoid = function (
    _x,
    _a,
    _b
  ) {
  ...
  p5.Ease.prototype.doubleEllipticOgee = function (_x, _a, _b) {
    ...
    if (_x <= _a) {
      ...
      _y = 1 - ((1 - _b) / (1 - _a)) * sqrt(sq(1 - _a) - sq(_x - _a));
    }
    return _y;
  };
  p5.Ease.prototype.doubleCubicOgee = function (_x, _a, _b) {
    ...
    var _y = 0;
    if (_x <= _a) {
      _y = _b - _b * pow(1 - _x / _a, 3);
    } else {
      _y = _b + (1 - _b) * pow((_x - _a) / (1 - _a), 3);
    }
    return _y;
  };
  ...
  p5.Ease.prototype.doubleSquircularSigmoid = function (_x, _a, _n) {
    ...
    var _y = 0;
    if (_x <= _a) {
      _y = _a - pow(pow(_a, pwn) - pow(_x, pwn), 1 / pwn);
    } else {
      _y = _a + pow(pow(1 - _a, pwn) - pow(_x - 1, pwn), 1 / pwn);
    }
    return _y;
  };
  p5.Ease.prototype.doubleQuadraticBezier = function (_x, _a, _b, _c, _d) {
    ...
    ymid = constrain(ymid, Number.EPSILON, 1 - Number.EPSILON);
    var _y = 0;
    var om2a;
    var t;
    var xx;
  ...
  p5.Ease.prototype.doubleEllipticSigmoid = function (_x, _a, _b) {
    ...
    if (_x <= _a) {
      if (_a <= 0) {
        ...
        _y = _b * (1 - sqrt(sq(_a) - sq(_x)) / _a);
      }
    } else {
      if (_a >= 1) {
        _y = 1;
      } else {
        _y = _b + ((1 - _b) / (1 - _a)) * sqrt(sq(1 - _a) - sq(_x - 1));
      }
    }
    return _y;
  };
  p5.Ease.prototype.doubleCubicOgeeSimplified = function (_x, _a, _b) {
    ...
    if (_x <= _a) {
      if (_a <= 0) {
        ...
        var val = 1 - _x / _a;
        _y = _b * _x + (1 - _b) * _a * (1 - val * val * val);
      }
    } else {
      if (_a >= 1) {
        _y = 1;
  ...
  p5.Ease.prototype.normalizedErf = function (_x) {
    var erfBound = 2;
    var erfBoundNorm = 0.99532226501;
    var z = map(_x, 0, 1, 0 - erfBound, erfBound);
    var z2 = z * z;
    var a = (8 * (PI - 3)) / (3 * PI * (4 - PI));
    var _y = sqrt(1 - exp(0 - z2 * ((a * z2 + 4 / PI) / (a * z2 + 1))));
    if (z < 0) _y = 0 - _y;
    _y /= erfBoundNorm;
    _y = (_y + 1) / 2;
    return _y;
  ...
  p5.Ease.prototype.circularFillet = function (_x, _a, _b, _c) {
    ...
    var min_param_a = 0 + Number.EPSILON;
    var max_param_a = 1 - Number.EPSILON;
    var min_param_b = 0 + Number.EPSILON;
    var max_param_b = 1 - Number.EPSILON;
    _a = constrain(_a, min_param_a, max_param_a);
    _b = constrain(_b, min_param_b, max_param_b);
    var R = _c;
    ...
    computefillet: {
      ...
      mPx = (p3x + p4x) / 2;
      mPy = (p3y + p4y) / 2;
      d1 = linetopoint(a1, b1, c1, mPx, mPy);
      if (d1 == 0) {
        break computefillet;
      }
      mPx = (p1x + p2x) / 2;
      mPy = (p1y + p2y) / 2;
      d2 = linetopoint(a2, b2, c2, mPx, mPy);
      if (d2 == 0) {
        break computefillet;
      }
      var c1p, c2p, d;
      ...
      d = a1 * b2 - a2 * b1;
      var pCx = (c2p * b1 - c1p * b2) / d;
      var pCy = (c1p * a2 - c2p * a1) / d;
      var pAx = 0;
      var pAy = 0;
      var pBx = 0;
      var pBy = 0;
  ...
  p5.Ease.prototype.circularArcThroughAPoint = function (_x, _a, _b) {
    ...
    function calcCircleFrom3Points(pt1x, pt1y, pt2x, pt2y, pt3x, pt3y, m) {
      ...
      if (abs(aSlope - bSlope) <= Number.EPSILON) {
        return;
      }
      m.centerX =
        ...
        (2 * (bSlope - aSlope));
      m.centerY =
        (-1 * (m.centerX - (pt1x + pt2x) / 2)) / aSlope + (pt1y + pt2y) / 2;
      m.dRadius = sqrt(sq(m.centerX - pt1x) + sq(m.centerY - pt1y));
    }
    if (!isPerpendicular(pt1x, pt1y, pt2x, pt2y, pt3x, pt3y))
      calcCircleFrom3Points(pt1x, pt1y, pt2x, pt2y, pt3x, pt3y, m);
    else if (!isPerpendicular(pt1x, pt1y, pt3x, pt3y, pt2x, pt2y))
      calcCircleFrom3Points(pt1x, pt1y, pt3x, pt3y, pt2x, pt2y, m);
    else if (!isPerpendicular(pt2x, pt2y, pt1x, pt1y, pt3x, pt3y))
      calcCircleFrom3Points(pt2x, pt2y, pt1x, pt1y, pt3x, pt3y, m);
    else if (!isPerpendicular(pt2x, pt2y, pt3x, pt3y, pt1x, pt1y))
      calcCircleFrom3Points(pt2x, pt2y, pt3x, pt3y, pt1x, pt1y, m);
    else if (!isPerpendicular(pt3x, pt3y, pt2x, pt2y, pt1x, pt1y))
      calcCircleFrom3Points(pt3x, pt3y, pt2x, pt2y, pt1x, pt1y, m);
    else if (!isPerpendicular(pt3x, pt3y, pt1x, pt1y, pt2x, pt2y))
      calcCircleFrom3Points(pt3x, pt3y, pt1x, pt1y, pt2x, pt2y, m);
    else {
      return 0;
    }
  ...
  p5.Ease.prototype.cubicBezier = function (_x, _a, _b, _c, _d) {
    ...
    var min_param_b = 0;
    var max_param_b = 1;
    var min_param_c = 0 + Number.EPSILON;
    var max_param_c = 1 - Number.EPSILON;
    var min_param_d = 0;
    var max_param_d = 1;
    _a = constrain(_a, min_param_a, max_param_a);
    _b = constrain(_b, min_param_b, max_param_b);
    ...
    for (var i = 0; i < nRefinementIterations; i++) {
      var currentx =
        A * (currentt * currentt * currentt) +
        B * (currentt * currentt) +
        C * currentt +
        D;
      var currentslope =
        1 / (3 * A * currentt * currentt + 2 * B * currentt + C);
      currentt -= (currentx - _x) * currentslope;
      currentt = constrain(currentt, 0, 1);
    }
    var _y =
      E * (currentt * currentt * currentt) +
      F * (currentt * currentt) +
      G * currentt +
      H;
    return _y;
  ...
  p5.Ease.prototype.doubleSquircularOgee = function (_x, _a, _n) {
    ...
    var _y = 0;
    if (_x <= _a) {
      _y = pow(pow(_a, pown) - pow(_x - _a, pown), 1 / pown);
    } else {
      _y = 1 - pow(pow(1 - _a, pown) - pow(_x - _a, pown), 1 / pown);
    }
    return _y;
  };
  p5.Ease.prototype.generalSigmoidLogitCombo = function (_x, _a, _b) {
    ...
    if (_a < 0.5) {
      var dy = _b - 0.5;
      _y = dy + this.normalizedLogit(_x, 1 - 2 * _a);
    } else {
      var dx = _b - 0.5;
      _y = this.normalizedLogitSigmoid(_x + dx, 2 * (_a - 0.5));
    }
    _y = constrain(_y, 0, 1);
    return _y;
  };
  p5.Ease.prototype.normalizedLogitSigmoid = function (_x, _a) {
    ...
    _a = 1 / (1 - _a) - 1;
    _a = emph * _a;
    var _y = 1 / (1 + exp(0 - (_x - 0.5) * _a));
    var miny = 1 / (1 + exp(0.5 * _a));
    var maxy = 1 / (1 + exp(-0.5 * _a));
  ...
  p5.Ease.prototype.normalizedLogit = function (_x, _a) {
    ...
    _a = 1 / (1 - _a) - 1;
    _a = emph * _a;
    var minx = 1 / (1 + exp(0.5 * _a));
    var maxx = 1 / (1 + exp(-0.5 * _a));
  ...
  p5.ArrayEval.prototype.eval = function (_evalstr, _l1) {
    ...
    var multi = 0;
    var e;
    if (Array.isArray(_evalstr)) multi = 1;
    var _dest;
  ...
  p5.Filt.prototype.tick = function (_x) {
    this.xn = _x;
    this.yn =
      (this.b0 / this.a0) * this.xn +
      (this.b1 / this.a0) * this.xpn +
      (this.b2 / this.a0) * this.xppn -
      (this.a1 / this.a0) * this.ypn -
      (this.a2 / this.a0) * this.yppn;
    this.xppn = this.xpn;
    this.xpn = this.xn;
    this.yppn = this.ypn;
    this.ypn = this.yn;
  ...
  p5.Filt.prototype.coeffs = function (_a0, _b0, _b1, _b2, _a1, _a2) {
    if (arguments.length != 6) {
      ...
      console.log(
        "     a2 -> gain for previous previous output (y[n-2] term)."
      );
      console.log(
        "when working with 5-coefficient biquad formulae set a0 to 1.0."
      );
      console.log(
        "n.b. some systems will refer to y terms as 'b' and x terms as 'a'."
      );
    } else {
      this.a0 = _a0;
      this.b0 = _b0;
  ...
  p5.FastFourierTransform.prototype.calculateFrequency = function () {
    ...
    for (i in this.frequency) {
      this.frequency[i] =
        this.bandwidth * i +
        (this.runningphase[i] * this.sampleRate) / (TWO_PI * this.hopSize);
    }
  };
  p5.FastFourierTransform.prototype.forward = function (_buffer) {
    var k = floor(log(this.bufferSize) / Math.LN2);
    if (pow(2, k) !== this.bufferSize) {
      throw "buffer size must be a power of 2.";
    }
    if (this.bufferSize !== _buffer.length) {
      throw (
        "buffer is not the same size as defined FFT. FFT: " +
        bufferSize +
        " buffer: " +
        buffer.length
      );
    ...
    while (halfSize < this.bufferSize) {
      ...
      for (var fftStep = 0; fftStep < halfSize; fftStep++) {
        ...
        while (i < this.bufferSize) {
          o = i + halfSize;
          tr =
            currentPhaseShiftReal * this.real[o] -
            currentPhaseShiftImag * this.imag[o];
          ti =
            currentPhaseShiftReal * this.imag[o] +
            currentPhaseShiftImag * this.real[o];
    ...
    if (this.doSpectrum) this.calculateSpectrum();
  };
    ...
    while (halfSize < this.bufferSize) {
      ...
      for (var fftStep = 0; fftStep < halfSize; fftStep++) {
        ...
        while (i < this.bufferSize) {
          off = i + halfSize;
          tr =
            currentPhaseShiftReal * _real[off] -
            currentPhaseShiftImag * _imag[off];
          ti =
            currentPhaseShiftReal * _imag[off] +
            currentPhaseShiftImag * _real[off];
  ...
  p5.prototype.besselI0 = function (_x) {
    ...
    if (_x == 0) {
      ...
      numerator =
                        ...
                        (z *
                          (z *
                            (z *
                              (z *
                                (z *
                                  (z * 210580722890567e-37 +
                                    3.80715242345326e-20) +
                                  4794402575483e-29) +
                                435125971262668e-28) +
                              30093112711296e-24) +
                            1.60224679395361e-8) +
        ...
        1440482982.27235;
      denominator =
        z * (z * (z - 3076.46912682801) + 3476263.32405882) - 1440482982.27235;
    }
    return -numerator / denominator;
  };
  p5.prototype.fplot = function (_array, _css) {
    var a;
    if (!Array.isArray(_array)) a = [_array];
    if (Array.isArray(_array)) a = _array;
    if (_array.constructor == Float32Array)
      a = Array.prototype.slice.call(_array);
    if (_array.constructor == Float64Array)
      a = Array.prototype.slice.call(_array);
    var TERMWIDTH = 80;
    var TERMHEIGHT = 23;
    var out = [];
